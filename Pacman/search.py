...
def depthFirstSearch(problem):
    ...
    #VISITED STATES
    visited = []
    #Record the path you followed from starting
    path = [] 

    #Check if starting point and final point are same
    ...
    while True:
      ...
      #If not found. get the successors
...
def breadthFirstSearch(problem):
    ...
    #Start with the first state
    qxy.push((problem.getStartState(),[]))
    ...
    while True:
      ...
      if succ:
        for item in succ:
          if item[0] not in visited and item[0] not in (state[0] for state in qxy.list):
            if problem.isGoalState(item[0]):
              return path + [item[1]]
            newPath = path + [item[1]]
            qxy.push((item[0],newPath))
